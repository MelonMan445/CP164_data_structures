...
class Queue:
    ...
    def __eq__(self, target):
        ...
        equals = self._count == target._count  # Start by checking size

        if equals:  # Only check elements if sizes match
            for i in range(self._count):
                ...
                if self._values[index_self] != target._values[index_target]:
                    equals = False  # Found a mismatch, queues are not equal
    ...
    def insert(self, value):
        ...
        if self._front is None:  # Reset _front when queue was empty
            self._front = 0
